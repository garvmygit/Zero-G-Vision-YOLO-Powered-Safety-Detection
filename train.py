EPOCHS = 20  # Reduced for faster training - can increase if needed
MOSAIC = 0.5  # Reduced mosaic for faster training
OPTIMIZER = 'AdamW'
MOMENTUM = 0.9
LR0 = 0.01  # Learning rate
LRF = 0.0001
SINGLE_CLS = False
BATCH_SIZE = 8  # Optimized batch size for CPU training
IMG_SIZE = 416  # Reduced image size for faster training
WORKERS = 4  # Add workers for faster data loading
...
if __name__ == '__main__': 
    ...
    results = model.train(
        ...
        epochs=args.epochs,
        device='cpu',  # Use CPU (GPU not available)
        ...
        batch=args.batch,  # Reduced batch size for CPU
        imgsz=args.imgsz,  # Reduced image size
        workers=args.workers,  # Add workers for faster data loading
